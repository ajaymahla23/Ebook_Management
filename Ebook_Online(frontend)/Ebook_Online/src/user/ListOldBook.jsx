...
function ListOldBook() {
  const [bookDetails, setBookDetails] = useState([]);

  useEffect(() => {
    document.title = "Old Book List";
    getBookDetailData();
  }, []);
  ...
  const getBookDetailData = () => {
    ...
    if (!token) return;
    axios
      .get(`${base_url}/ebook/user/old-books`, {
        headers: {
                ...
                bookDetails.map((detail) => (
                  <tr key={detail.bookId}>
                    ...
                    <td className="text-center">
                      {detail.bookImg && (
                        <img
                          src={`${base_url}/ebook/admin/getBookImg/${detail.bookImg}`}
                          height="70"
                          width="70"
                        />
                      )}
